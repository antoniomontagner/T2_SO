...
// increment index cyclic array
#define add_position_i(i, max) (i+1) % (max-1)
#define add_position_clock(i) i == NR_FRAMES-1 ? 0 : i+1

typedef struct clock_frame {
	int qtd;
	int position_clock;
	frame_t *frames[NR_FRAMES];
} clock_frame_t;


// initializing clock
clock_frame_t clock1;
clock_frame_t clock2;

PRIVATE void pop_frame_clock(clock_frame_t *clock_now, int i) {
	int position = i;
	while (position < clock_now->qtd) {
		frame_t *frame_now1 = clock_now->frames[position + 1];
		clock_now->frames[position] = frame_now1;
		position++;
	}
...
PRIVATE int find_i (clock_frame_t *clock_now, int i) {
	frame_t *frame_now = clock_now->frames[i];
	
	for (int j = 0; j < NR_FRAMES; j++) {
		if (frame_now == &frames[j])
			return j;
	}
...
PRIVATE int allocf(void)
{
	...
	// Part 1 -> if any frame is free, get to it
	// while it has't allocated everything, there's something free
	if (clock1.qtd + clock2.qtd < NR_FRAMES)
		for (i = 0; i < NR_FRAMES; i++) {
			frame_t *frame_now = clock1.frames[i];

			if (frame_now == NULL || frame_now->count == 0) {
				// add in clock1
				frame_t *frame_now = frames + i;
				clock1.frames[clock1.qtd] = frame_now;
				clock1.qtd++;
	...
	// Part 2 		-> Double clock algorithm
	// Part 2.a		-> Loop clock1
	i = clock1.position_clock;
	while (TRUE) {
		if (clock1.qtd + clock2.qtd > NR_FRAMES) {
			kpanic("Clocks allocation are bigger than NR_FRAMES");
			return (-1);
		}

		// get frame analysed now
		frame_t *frame_now = clock1.frames[i];

		// get page table entry
		struct pte *table = getpte(curr_proc, frame_now);

		// local replacement policy
		if (frame_now->owner == curr_proc->pid) {
			// if R = 0 and M = 0, then substitute the page and put the clock1 up
			if (table->accessed == 0 && table->dirty == 0) {				
				swap_out(curr_proc, frame_now->addr);

				// add_position_clock(clock1);
				clock1.position_clock = add_position_clock(clock1.position_clock);

				i = find_i(&clock1, i);
				goto found;
			}

			// if R = 1 and M = 0, put R = 0 and clock1 up
			if (table->accessed == 1 && table->dirty == 0) {
				table->accessed = 0;

				clock1.position_clock = add_position_clock(clock1.position_clock);
			}

			// if M = 1, put R = 0, remove this clock1,
			// put in clock2 and continue the loop
			if (table->dirty == 1) {
				// put R = 0
				table->accessed = 0;

				// remove element from clock1
				pop_frame_clock(&clock1, i);

				// pass frame_now from clock1 to clock2
				clock2.frames[clock2.qtd] = frame_now;
				clock2.qtd++;
			}
		}
		
		i = add_position_i(i, clock1.qtd);

		// if it turned the whole clock, break
		if (i == clock1.position_clock) break;
	}

	// Part 2.b	-> Loop clock2
	// classic clock algorithm in clock2
	i = clock2.position_clock;
	while (TRUE) {
		// get frame analysed now
		frame_t *frame_now = clock2.frames[i];
		...
		if (frame_now->owner == curr_proc->pid) {
			...
			// if R = 1, put to 0 and advance clock2 pointer
			if (table->accessed == 1) {
				table->accessed = 0;

				// add_position_clock(clock2);
				clock2.position_clock = add_position_clock(clock2.position_clock);

				i = add_position_i(i, clock2.qtd);
				continue;
			}
